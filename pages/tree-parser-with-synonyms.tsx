...
const TreeParserPage = () => {
    ...
    const onSubmit = () => {
        setWarning("");
        setLoading(true);
        jsonFetch("/api/tree-parser-with-synonyms", { firstUrl, rootUrls: rootUrls.filter(rootUrl => rootUrl), numberOfKeywords, linkLimit })
    ...
    return (
        <Layout title="Tree Parser With Synonyms">
            { loading && <Spinner />}
            {
                showResults && firstPageData && treePageDatasObjects
                    ? <TreeView firstPageData={firstPageData} treePageDatasObjects={treePageDatasObjects} firstPageUrl={firstUrl} synonymCheck={true} />
                    : <Form onSubmit={onSubmit} formTitle="Calculate Keywords On Link Tree With Synonyms" warning={warning}>
